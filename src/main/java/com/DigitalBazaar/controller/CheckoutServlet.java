        ...
        for (int i = 0; i < productNames.length; i++) {
            try {
                ...
                try (Connection conn2 = DBConfig.getDbConnection();
                    ...
                    ps.setInt(1, qty);       // ← was "quantity", now "qty"
                    ps.setInt(2, p.getId());
                    ps.setInt(3, qty);       // ← was "quantity", now "qty"
                    ps.executeUpdate();
                } catch (Exception e) {
